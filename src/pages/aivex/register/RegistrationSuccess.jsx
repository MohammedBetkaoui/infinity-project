...
export default function RegistrationSuccess({ team, memberCount, reference, reduced, onReset }) {
  return (
    <motion.div className="axr-success" role="status"
      ...
      <dl className="axr-success-record">
        <div><dt>Team</dt><dd>{team.name.trim()}</dd></div>
        <div><dt>Members</dt><dd>{memberCount} students</dd></div>
        <div><dt>Contact</dt><dd>{team.email.trim()}</dd></div>
        {reference && <div><dt>Reference</dt><dd className="axr-success-ref">{reference}</dd></div>}
      </dl>

      <p className="axr-success-note">
        The next AIVEX updates will be sent to the team email above. The challenge itself stays sealed until the competition opens.
