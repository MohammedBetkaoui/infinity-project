...
function actionDialogLabels(isArabic, t) {
  return isArabic ? { close: 'إغلاق', cancel: t('Cancel'), confirm: t('Confirm action'), eyebrow: t('Administrative action'), description: t('This decision updates the real AIVEX file and is recorded with the authenticated administrator.'), required: t('Required'), internalReason: t('Internal reason'), reasonPlaceholder: t('Explain the decision for your colleagues…'), reasonError: t('Add an internal reason before confirming.'), submitError: t('The action could not be completed. Please try again.') } : { description: 'This decision updates the real AIVEX file and is recorded with the authenticated administrator.' }
}
...
function SecureViewer({ team, document: file, onClose, onUpdated, locale, loadDocument, act }) {
  ...
  const [note, setNote] = useState(file.verificationNote || '')
  ...
  const submitReview = async (action) => {
    ...
    setSaving(true)
    const result = await act(team.ref, { action, expectedUpdatedAt: team.updatedAt, reason: note, payload: { documentKey: file.id } })
    setSaving(false)
  ...
  return <Modal open wide onClose={saving ? () => {} : onClose} closeLabel={isArabic ? 'إغلاق' : 'Close'} title={t('Confidential document')} eyebrow={t('Restricted review · Access recorded')} footer={<><span className="adm-viewer-timer"><LockKeyhole size={14}/>{t('Auto-close in')} <Ltr>{Math.floor(remaining / 60)}:{String(remaining % 60).padStart(2, '0')}</Ltr></span><Button variant="secondary" onClick={onClose} disabled={saving}>{t('Close viewer')}</Button><Button variant="danger" onClick={() => submitReview('invalidate_document')} disabled={saving || loading}>{t('Mark invalid')}</Button><Button onClick={() => submitReview('verify_document')} disabled={saving || loading || file.status === 'Verified'} icon={<CheckCircle2 size={16}/>}>{t(file.status === 'Verified' ? 'Already verified' : saving ? 'Saving…' : 'Mark as verified')}</Button></>}>
    ...
    <label className="adm-form-field"><span>{t('Verification note')} <em>{t('Required')}</em></span><textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder={t('Record what was checked without copying identity data…')}/></label>
...
export function AivexDetailPage() {
  ...
  const decision = (title, actionName, danger = false, extra = {}) => setAction({ key: actionName, title: t(title), danger, ...extra })
  const submitAction = async (values) => {
    const result = await act(team.ref, {
      action: action.key, expectedUpdatedAt: team.updatedAt, reason: values.reason,
  ...
  const saveCorrections = async (event) => {
    ...
    const result = await act(team.ref, {
      ...
      reason: data.get('reason'), payload: { items, message: data.get('message'), deadline: data.get('deadline') },
    })
    setBusy(false)
  ...
  return <div className={`adm-page adm-aivex-page adm-team-page ${isArabic ? 'is-arabic' : ''}`} dir={dir} lang={language}>
    ...
    })}</div></section><aside className="adm-panel adm-decision-panel"><SectionHeading index="06" title={t('File decision')}/><Progress value={team.completeness}/><p>{t('Each decision requires an internal reason and is recorded in the file history.')}</p>{allowed.has('start_review') && <Button variant="secondary" onClick={() => decision('Move file to review', 'start_review')}>{t('Move to review')}</Button>}{allowed.has('approve_registration') && <Button variant="secondary" disabled={team.registration === 'Approved'} onClick={() => decision('Approve registration', 'approve_registration')}>{t('Approve registration')}</Button>}{allowed.has('request_corrections') && <Button variant="secondary" onClick={() => { setCorrectionError(''); setCorrections(true) }}>{t('Request corrections')}</Button>}{allowed.has('validate_file') && <Button disabled={!team.canValidate || team.document === 'Validated'} onClick={() => decision('Validate AIVEX file', 'validate_file')} icon={<ShieldCheck size={17}/>}>{t(team.document === 'Validated' ? 'File validated' : 'Validate file')}</Button>}{!team.canValidate && <p className="adm-validation-help"><LockKeyhole size={15}/>{t('Complete all checks, approve registration and verify every identity file plus the active signed document to unlock validation.')}</p>}<div className="adm-danger-actions">{allowed.has('reject_registration') && <button onClick={() => decision('Reject registration', 'reject_registration', true)}>{t('Reject registration')}</button>}{allowed.has('cancel_registration') && <button onClick={() => decision('Cancel registration', 'cancel_registration', true)}>{t('Cancel registration')}</button>}</div>{team.correctionRequest && <div className="adm-correction-summary"><b>{t('Current correction request')}</b><p>{team.correctionRequest.message}</p><small>{t('Due')} <Ltr>{team.correctionRequest.deadline}</Ltr></small><ul>{team.correctionRequest.items.map((item) => <li key={item}>{t(item)}</li>)}</ul></div>}</aside></div>}
