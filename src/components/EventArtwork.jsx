  ...
  return (
    <div className={`event-artwork artwork-${variant}`} aria-hidden="true" data-aivex-network={variant === 'ai' ? '' : undefined}>
      ...
      {variant === 'design' && <>
        ...
        <p className="poster-foot">From the first sketch to an identity.</p>
      </>}
      {variant === 'ramadan' && <>
        ...
        <p className="poster-foot">Time for a conversation.</p>
      </>}
      {variant === 'access' && <>
        <div className="access-prompt">&gt; the journey starts here_</div>
        <div className="poster-title">ACCESS<span>0</span></div>
        <div className="access-door"><span>↳</span></div>
        <p className="poster-foot">One first step. Every possibility.</p>
      </>}
    </div>
  )
}
